...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ==================================================
       RESET / FILL ANOTHER FORM (ZERO RELOAD RESET)
    ================================================== */
    const fillBtn = document.getElementById('fillAnotherBtn');
    if (fillBtn) {
        fillBtn.addEventListener('click', () => {
            ...
            // 2. Hide success, show form & stepper
            successScreen.classList.remove('active');
            if (form) form.style.display = 'block';
            const stepperWrap = document.querySelector('.stepper-wrap');
            if (stepperWrap) stepperWrap.style.display = 'block';

            // 3. Reset form data and steps
            if (form) form.reset();
            current = 0;

            // 4. Reset dynamic tables (Tabulator)
            if (window.formTables) {
                Object.keys(window.formTables).forEach(id => {
                    const tbl = window.formTables[id];
                    // Clear all data or replace with empty rows
                    const data = tbl.getData();
                    data.forEach(row => {
                        let emptyRow = { _row_idx: row._row_idx };
                        Object.keys(row).forEach(key => {
                            if (key !== '_row_idx') emptyRow[key] = "";
                        });
                        tbl.updateRow(row._row_idx, emptyRow);
                    });
                });
                if (window.syncAllTables) window.syncAllTables();
            }

            // 5. Update UI state (stepper progress, active step)
            updateUI(); 
            initPills(); // Refresh pill visuals
