...
<form action="{{ route('admin.questions.update', $question->id) }}" method="POST">
    ...
    <div class="card" style="margin-bottom: 2rem;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1.5rem;">
            ...
            <div style="display:flex; align-items:flex-end;">
                <label style="display:flex; align-items:center; cursor:pointer;">
                    <input type="checkbox" name="meta_params[has_reason]" value="1" {{ !empty($question->meta_params['has_reason']) ? 'checked' : '' }} style="margin-right:0.5rem;" onchange="document.getElementById('reason-label-container').style.display = this.checked ? 'block' : 'none'">
                    Enable 'શા માટે?' (Why?)
                </label>
            </div>
        </div>
